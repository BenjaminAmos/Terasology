...
public class LwjglCanvasRenderer implements TerasologyCanvasRenderer, PropertyChangeListener {
    ...
    public void preRender() {
        ...
        requestedCropRegion = RectUtility.createFromMinAndSize(0, 0, Display.getWidth(), Display.getHeight());
    ...
    @Override
    public void drawMesh(Mesh mesh, Material material, Rectanglei drawRegion, Rectanglei cropRegion, Quat4f rotation, Vector3f offset, float scale, float alpha) {
        ...
        Matrix4f translateTransform = new Matrix4f(BaseQuat4f.IDENTITY,
                new Vector3f((drawRegion.minX + drawRegion.lengthX() / 2) * uiScale,
                    (drawRegion.minY + drawRegion.lengthY() / 2) * uiScale, 0), 1);
    ...
    public FrameBufferObject getFBO(ResourceUrn urn, Vector2i size) {
        ...
        if (frameBufferObject == null || !Assets.getTexture(urn).isPresent()) {
            ...
            frameBufferObject = new LwjglFrameBufferObject(urn, JomlUtil.from(size));
    ...
    @Override
    public void drawTexture(UITextureRegion texture, Color color, ScaleMode mode, Rectanglei absoluteRegion,
        ...
        org.joml.Vector2f scale = mode.scaleForRegion(absoluteRegion, texture.getWidth(), texture.getHeight());
        Rectanglef textureArea = texture.getRegion();
        Mesh mesh = billboard;
        switch (mode) {
            case TILED: {
                TextureCacheKey key = new TextureCacheKey(texture.size(), absoluteRegion.lengths(new org.joml.Vector2i()));
    ...
    @Override
    public void drawTextureBordered(UITextureRegion texture, Rectanglei region, Border border, boolean tile, float ux, float uy, float uw, float uh, float alpha) {
        if (!((org.terasology.rendering.assets.texture.TextureRegion)texture).getTexture().isLoaded()) {
            return;
        }
        ...
        TextureCacheKey key = new TextureCacheKey(textureSize, region.lengths(new org.joml.Vector2i()), border, tile);
        usedTextures.add(key);
        ...
        if (mesh == null || mesh.isDisposed()) {
            ...
            if (border.getTop() != 0) {
                ...
                if (tile) {
                    addTiles(builder, RectUtility.createFromMinAndSize(border.getLeft(), 0, centerHoriz, border.getTop()), Rect2f.createFromMinAndMax(left, 0, right, top),
            ...
            if (border.getLeft() != 0) {
                if (tile) {
                    addTiles(builder, RectUtility.createFromMinAndSize(0, border.getTop(), border.getLeft(), centerVert), Rect2f.createFromMinAndMax(0, top, left, bottom),
    ...
    private void addTiles(MeshBuilder builder, Rectanglei drawRegion, Rect2f subDrawRegion, Vector2i textureSize, Rect2f subTextureRegion) {
        ...
        for (int tileY = 0; tileY < vertTiles; tileY++) {
            for (int tileX = 0; tileX < horizTiles; tileX++) {
                ...
                float vertLeft = subDrawRegion.minX() + subDrawRegion.width() * Math.max((float) left / drawRegion.lengthX(), 0);
                float vertTop = subDrawRegion.minY() + subDrawRegion.height() * Math.max((float) top / drawRegion.lengthY(), 0);
                float vertRight = subDrawRegion.minX() + subDrawRegion.width() * Math.min((float) (left + tileW) / drawRegion.lengthX(), 1);
                float vertBottom = subDrawRegion.minY() + subDrawRegion.height() * Math.min((float) (top + tileH) / drawRegion.lengthY(), 1);
                ...
                float texCoordRight = subTextureRegion.minX() + subTextureRegion.width() * (Math.min(left + tileW, drawRegion.lengthX()) - left) / tileW;
                float texCoordBottom = subTextureRegion.minY() + subTextureRegion.height() * (Math.min(top + tileH, drawRegion.lengthY()) - top) / tileH;
