...
public class TerasologyCanvasImpl extends CanvasImpl implements PropertyChangeListener {
    ...
    public void drawMesh(Mesh mesh, Material material, Rectanglei region, Quat4f rotation, Vector3f offset, float scale) {
        if (material == null) {
    ...
    public void drawMesh(Mesh mesh, UITextureRegion texture, Rectanglei region, Quat4f rotation, Vector3f offset, float scale) {
    ...
    private final class SubRegionFBOImpl implements SubRegion {
        ...
        private SubRegionFBOImpl(ResourceUrn uri, Vector2i size) {
            ...
            state = new CanvasState(state, RectUtility.createFromMinAndSize(new Vector2i(), size));
            fbo.bindFrame();
        }
